...
exp.in = data.frame(fread("/media/d1/Syn_out_KG/matqtl/inputs/gene_expression_cqn.txt"))
...
fwrite(f5, "/media/d1/Syn_out_KG/F5_expression.csv", row.names = T)

cm = covariates_mat
output.creater(exp.df = t(exp.vals), cv.df=cm, results.dir="/media/d1/Syn_out_KG/", tissue="Synovium", 
                 cv.names=c(paste0("EV", 1:4), paste0("PEER", 1:4)), rs.plotter=c())
